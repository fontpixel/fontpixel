...
用法：python -m fbf.build --fonts fonts --out site/public/data \
...
from fbf.cache import FamilyCache, data_dir_hash, family_key
...
from fbf.familymeta import FamilyMeta, VariantDesc, load_family_meta, resolve_variant
from fbf.glyphpack import write_packs
from fbf.licenses import detect_license
from fbf.metrics import InkMetrics, claimed_size, compute_ink, is_monospaced
...
from fbf.samples import SAMPLES
from fbf.variants import search_text
...
def _build_family(
    ...
    dl_entries = build_downloads(
        ...
        copyright_line=f"{meta.name} — {license_info.name}"
    ...
    entry = {
        ...
        "added": meta.added,
        # 简繁展开后的可搜索文本：搜「东云」也能命中「東雲」
        "searchText": search_text(meta.name, meta.name_zh, meta.author, slug,
                                  " ".join(meta.vibes)),
    }
    ...
    from fbf.emit import cps_to_runs
...
def build(fonts_dir: Path, site_data: Path, downloads_dir: Path, cache_dir: Path,
    ...
    from fbf.emit import write_charsets_json, write_intervals
