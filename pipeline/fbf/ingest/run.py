...
用法：python -m fbf.ingest.run --manifest ingest/manifest.toml \
...
def _ingest_family(fam: dict, src_root: Path, dest_root: Path,
    ...
    if not toml_path.exists():
        ...
        lic = fam.get("license")
        if lic:
            text += _LICENSE_TEMPLATE.format(
                spdx=lic.get("spdx", ""),
                name=lic.get("name", ""),
                commercial=str(bool(lic.get("commercial", True))).lower(),
                note=lic.get("note", "").replace('"', "'"),
            )
            if lic.get("file"):
                text += f'file = "{lic["file"]}"\n'
...
def _write_report(report: IngestReport, path: Path, data: dict) -> None:
    lines = [
        "# 导入报告",
        "",
        f"生成时间：{TODAY}（由 fbf.ingest.run 生成，重跑覆盖）",
        "",
