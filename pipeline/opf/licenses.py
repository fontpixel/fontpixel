...
def detect_license(
    ...
    if toml_override:
        ...
        return LicenseInfo(
            ...
            name_en=toml_override.get("name_en", "") or toml_override.get("name", "")
                    or _NAMES.get(spdx or "", spdx or ""),
            note_en=toml_override.get("note_en", ""),
        )
    ...
    if hint:
        return LicenseInfo(
            ...
            name_en=_NAMES.get(hint, hint),
            note_en="Inferred from embedded font properties; unconfirmed.",
        )

    return LicenseInfo(
        spdx=None, name="未识别", file=None, confidence="unknown"
    )
