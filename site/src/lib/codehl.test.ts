...
describe('highlightCode', () => {
  ...
  it('注释、字符串、数字、关键字各归各色', () => {
    ...
    expect(HEX(out[0])).toBe('6a9955'); // 注释
    expect(HEX(out[src.indexOf('const')])).toBe('569cd6'); // 关键字
    expect(HEX(out[src.indexOf('42')])).toBe('b5cea8'); // 数字
    expect(HEX(out[src.indexOf('"') + 1])).toBe('ce9178'); // 字符串内容
    expect(HEX(out[src.indexOf('fn(')])).toBe('dcdcaa'); // 调用名
    expect(out[src.indexOf('n =')]).toBeNull(); // 普通标识符用默认墨色
  });
  ...
  it('Python 井号注释与三引号字符串', () => {
    ...
    expect(HEX(out[chars.indexOf('b')])).toBe('ce9178'); // 跨行仍在字符串里
  ...
  it('Rust 生命周期不吞行', () => {
    ...
    const chars = Array.from(code);
    expect(HEX(out[chars.indexOf('7')])).toBe('b5cea8'); // 没被当成字符串
  });
