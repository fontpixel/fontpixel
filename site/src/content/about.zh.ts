...
export const aboutIntro =
  '免费开源位图（像素）字体把散落在各处的开源点阵字体收进同一间标本馆：拆开到每个字形，用同一把尺子量，数清每张字表的覆盖，再给你一个能随手打字的试写框。字体属于它们各自的作者；这里做的只是整理与陈列。';
...
export const aboutSections: AboutSection[] = [
  {
    ...
    heading: '收录标准',
    paragraphs: [
      '只收录自由许可字体。判定标准是严格的：授权必须明确允许任何人使用、复制、修改、再分发，并且允许商业使用；授权须明确适用于本站收录的这份文件，来源链可核验。仅写着「免费」「免费商用」「freeware」或只允许复制的字体不算，作者未完成迁移的「将来会改成开源许可」也不算。因此站上每一款字体都可放心商用——不存在需要单独筛选的例外。',
      '收录以 CJK 点阵字体为主。原生点阵格式（BDF、PCF，以及可无损转换的 OTB、kbitx 等）优先；部分字体作者只发布了矢量化的像素轮廓字体(TTF/OTF)，这类会按原生格点栅格化转回点阵，并在页面上标注「转制」与推导方式。',
